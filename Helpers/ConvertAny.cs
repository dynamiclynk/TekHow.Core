...
using TekHow.Core.Properties;

namespace TekHow.Core.Helpers
{
    /// <summary>
    ...
    public static class ConvertAny
    {
        ...
        /// <exception cref="RuntimeBinderException">Raised when the value cannot be converted to the specified type.</exception>
        ...
        private static ToType ConvertInternal<ToType>(object fromValue)
        {
            try
            {
                ...
                throw new InvalidCastException(string.Format(StringResources.InvalidCastException, fromValue.GetType().ToString(), typeof(ToType).ToString()));
            }
            catch
            {
                throw;
